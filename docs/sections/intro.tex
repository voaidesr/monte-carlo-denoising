...
\begin{definition}
    ...
    \end{equation}
    where $z(p)$ is the denoised pixel at point $p$, $y(q)$ is the noisy pixel at point $q$, $w(p, q)$ is the weight function that measures the similarity between patches centered at $p$ and $q$, and $C(p)$ is a normalizing factor given by:
    \begin{equation}
